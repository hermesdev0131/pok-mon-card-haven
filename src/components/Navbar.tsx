...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/[0.06] bg-background/70 backdrop-blur-xl">
      <div className="container mx-auto flex h-16 items-center justify-between gap-4 px-4">
        ...
        <div className="hidden flex-1 max-w-md md:block">
          <div className="relative">
            ...
            <Input placeholder="Buscar cartas..." className="pl-9 h-9 bg-white/[0.04] border border-white/[0.06] placeholder:text-muted-foreground/60 focus:border-accent/30 focus:ring-accent/20" />
          </div>
        </div>
      ...
      {mobileOpen && (
        <div className="border-t border-white/[0.06] bg-background/90 backdrop-blur-xl p-4 md:hidden">
          <div className="relative mb-4">
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
            <Input placeholder="Buscar cartas..." className="pl-9 bg-white/[0.04] border border-white/[0.06]" />
          </div>
          <nav className="flex flex-col gap-3">
            ...
            <Button size="sm" className="w-fit bg-accent text-accent-foreground hover:bg-accent/90" asChild>
